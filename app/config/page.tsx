...
export default function Config() {
  ...
  const { setOverbooking, setDuration } = userContext;
  ...
  const handleToggleWorkDay = (index: number) => {
    setWorkDays(prev => ({
      ...prev,
      [index]: { ...prev[index], isWorkDay: !prev[index].isWorkDay },
    }));
  };
  ...
  const handleAddException = async () => {
    ...
    try {
      ...
      setExceptions(prev => [...prev, { ...newException, id: responseData.id }]);
      setNewException({ id: Date.now(), date: "", isOpen: false, start: "", end: "", lunchStart: "", lunchEnd: "" });
  ...
  const handleSaveAppointmentConfig = async (newDuration?: number, newAllowDoubleBooking?: boolean) => {
    try {
      if (newDuration !== undefined) setLoadingAppointmentConfig(prev => ({ ...prev, duration: true }));
      if (newAllowDoubleBooking !== undefined) setLoadingAppointmentConfig(prev => ({ ...prev, doubleBooking: true }));
      ...
      await appointmentService.updateConfig(token, {
        ...
        overbooking: newAllowDoubleBooking ?? allowDoubleBooking,
      });
      ...
      if (newAllowDoubleBooking !== undefined) setOverbooking(newAllowDoubleBooking);

    } catch (err) {
      console.error("Erro ao salvar configurações de agendamento:", err);
      setAppointmentConfigError(err instanceof Error ? err.message : "Erro desconhecido");
      showNotification("Erro ao atualizar configurações", "error");
    } finally {
      if (newDuration !== undefined) setLoadingAppointmentConfig(prev => ({ ...prev, duration: false }));
      if (newAllowDoubleBooking !== undefined) setLoadingAppointmentConfig(prev => ({ ...prev, doubleBooking: false }));
    }
  };

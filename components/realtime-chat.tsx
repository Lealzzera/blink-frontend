...
export const RealtimeChat = ({
  ...
  >({});
  const [paginationByContact, setPaginationByContact] = useState<
  ...
  useEffect(() => {
    if (!token || !selectedContact) {
      if (stompClientRef.current?.active) {
        console.log(
          "STOMP: Desconectando cliente por falta de token ou contato selecionado."
        );
    ...
    console.log(
      `STOMP: Configurando cliente para a sala: ${selectedContact.roomName}`
    );

    const client = new Client({
      webSocketFactory: () =>
        new SockJS(`http://localhost:3003/wpp-socket/subscribe`),
      debug: (str) => {
          ...
          try {
            ...
            const newChatMessage: ChatMessage = {
              id:
                ...
                `${selectedContact.number}-${
                  receivedMessage.sent_at
                }-${Math.random()}`,
              text: receivedMessage.message_text,
              content: receivedMessage.message_text,
              user: {
                name: receivedMessage.from_me ? username : selectedContact.name,
              },
            ...
            setMessagesByContact((prev) => {
              ...
              if (currentMessages.some((m) => m.id === newChatMessage.id)) {
                console.log(
                  "STOMP: Mensagem duplicada recebida, ignorando.",
                  newChatMessage.id
                );
                return prev;
              }
            ...
            console.error(
              "STOMP: Erro ao processar mensagem recebida:",
              e,
              message.body
            );
          }
      ...
      onStompError: (frame) => {
        console.error(
          "STOMP: Erro do broker. Cabeçalho:",
          frame?.headers?.["message"],
          "Corpo:",
          frame?.body
        );
        setError(
          "Erro de conexão com o chat em tempo real. Tentando reconectar..."
        );
        setIsConnected(false);
      },
      onWebSocketError: (event) => {
        ...
        setError(
          "Erro na conexão WebSocket. Verifique o console para detalhes."
        );
        setIsConnected(false);
      },
      onWebSocketClose: () => {
  ...
  const loadMoreContacts = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${
          process.env.NEXT_PUBLIC_API_BASE || "http://localhost:3003/api/v1"
        }/chat/1/overview?page=${nextPage}`,
        {
    ...
    async (
      ...
      try {
        ...
        const res = await fetch(
          `${
            process.env.NEXT_PUBLIC_API_BASE || "http://localhost:3003/api/v1"
          }/chat/1/overview/${contactNumber}?page=${pageToLoad}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        if (res.ok) {
          ...
          if (data.length > 0) {
            ...
            setMessagesByContact((prev) => ({
              ...
              [contactNumber]: reset
                ? mappedMessages
                : [...mappedMessages, ...(prev[contactNumber] || [])],
            }));
  ...
  const loadMoreMessages = async () => {
    ...
    const { page, hasMore, loading } = paginationByContact[
      selectedContact.number
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      const { loading } = paginationByContact[selectedContact.number] || {
        loading: false,
      };
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        setMessagesByContact((prev) => {
          ...
          return {
            ...prev,
            [selectedContact.number]: [...curr, optimisticMessage],
          };
        });
        ...
        const response = await fetch(
          `${
            process.env.NEXT_PUBLIC_API_BASE || "http://localhost:3003/api/v1"
          }/message/whats-app/send-message`,
          {
            method: "POST",
            headers: {
        ...
        if (resData && resData.id) {
          setMessagesByContact((prev) => {
            ...
            return {
              ...prev,
              [selectedContact.number]: list.map((m) =>
                m.id === optimisticId
                  ? {
                      ...m,
                      id: resData.id,
                      createdAt: resData.sent_at ?? m.createdAt,
                    }
                  : m
              ),
            };
        ...
        setMessagesByContact((prev) => {
          ...
          return {
            ...prev,
            [selectedContact.number]: list.filter(
  ...
  const selectedPagination = selectedContact
    ? paginationByContact[selectedContact.number] || {
        hasMore: true,
        loading: false,
      }
    ...
    <div className={styles.container}>
            ...
            >
              <div className={styles.header}>
                ...
                <Switch
                  ...
                  defaultChecked={contact.scheduled}
                />
              </div>
              <div className={styles.body}>
                <Image
                  src={
                    contact.photo ||
                    `https://dummyimage.com/100x100/eee/.png&text=${contact.name.charAt(
                      0
                    )}`
                  }
                  alt={contact.name}
      ...
      <div className={styles.chat}>
        <div className={styles.chatHeader}>
            ...
            <div className={styles.chatHeaderContact}>
              <Image
                src={
                  ...
                  `https://dummyimage.com/100x100/eee/.png&text=${selectedContact.name.charAt(
                    0
                  )}`
                }
          ...
          <Switch
            className={styles.switch}
            defaultChecked={selectedContact?.scheduled}
          />
        </div>
